...
*Note*: Ordinal features (such as `education level`) are not always appropriate for one-hot encoding, as doing so may ignore their inherent order. Alternative encoding strategies may be more suitable; see Section [-@sec-ch6-encoding] for more details.
