...
```{r fig-ch12-net-reg, echo = FALSE, out.width = "38%", fig.cap = "A graphical representation of a regression model: input features and predictions are shown as nodes, with the coefficients represented as connections between the nodes."}
...
```{r fig-ch12-net-large, echo = FALSE, out.width = "80%", fig.cap = "Visualization of a multilayer neural network model with two hidden layers."}
...
```{r fig-ch12-active-fun-comparison, echo = FALSE, out.width  = '70%', fig.cap = "Comparison of common activation functions: sigmoid, tanh, and ReLU."}
...
To understand this, consider the simple example shown in @fig-ch12-net-reg. This basic network consists of:
...
By predicting which customers are most likely to subscribe, the bank can optimize its outreach strategy, reduce marketing costs, and improve campaign effectiveness—all while minimizing unnecessary or intrusive communication.
...
Note that ordinal features (such as `education level`) are not always appropriate for one-hot encoding, as doing so may ignore their inherent order. Alternative encoding strategies may be more suitable; see Section [-@sec-ch6-encoding] for more details.
...
#### Hands-On Practice: Nerual Network with the *bank* Dataset {.unnumbered}
...
#### Hands-On Practice: Nerual Network with the *adult* Dataset {.unnumbered}
...
34. Scale the numerical features using min-max scaling.
