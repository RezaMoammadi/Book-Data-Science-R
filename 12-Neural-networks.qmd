...
# Neural Networks: The Building Blocks of Artificial Intelligence {#sec-ch12-neural-networks}
...
In this chapter, we continue advancing through the *Data Science Workflow* introduced in Chapter [-@sec-ch2-intro-data-science]. So far, we have learned how to prepare and explore data, and apply classification methods—including *k-Nearest Neighbors* (Chapter [-@sec-ch7-classification-knn]) and *Naive Bayes* (Chapter [-@sec-ch9-bayes])—as well as regression models (Chapter [-@sec-ch10-regression]) and tree-based models (Chapter [-@sec-ch11-tree-models]). We have also discussed how to evaluate model performance (Chapter [-@sec-ch8-evaluation]).
...
-   It is also instructive to compare the neural network’s performance with simpler classifiers—such as logistic regression (Chapter [-@sec-ch10-regression]), k-nearest neighbors (Chapter [-@sec-ch7-classification-knn]), Naive Bayes (Chapter [-@sec-ch9-bayes]), or tree-based models (Chapter [-@sec-ch11-tree-models]). Doing so helps clarify when the added complexity of a neural network is justified.
