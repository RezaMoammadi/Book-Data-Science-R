...
So far, we have examined hypothesis tests that compare two groups, such as the two-sample t-test and the two-sample Z-test. But what if we want to compare a numerical outcome across more than two groups? For example, does the mean price of diamonds differ across cut categories? When the grouping variable has three or more levels, analysis of variance (ANOVA) provides a formal way to test whether at least one group mean differs from the others.
...
```{r fig-ch5-anova-concept, echo=FALSE, out.width="60%", fig.cap="Conceptual idea behind ANOVA. The method compares variability between group means with variability within groups. Larger between-group variability relative to within-group variability provides evidence that not all population means are equal."}
knitr::include_graphics("images/ch5_anova.png")
```

To illustrate ANOVA in R, we use the `diamonds` dataset from the **ggplot2** package. The response variable is `price`, and the grouping variable is `cut`, which has five levels: `"Fair"`, `"Good"`, `"Very Good"`, `"Premium"`, and `"Ideal"`. Diamond prices are right-skewed, so the visual comparison should be interpreted cautiously. In this introductory example, we keep the original price scale to focus on the basic ANOVA workflow.
...
The boxplot shows differences in the price distributions across cut categories. Visual inspection, however, cannot determine whether the observed differences in group means are statistically significant. ANOVA provides a formal test of this question.

For this example, let $\mu_1, \ldots, \mu_5$ denote the population mean prices for the five cut categories. The hypotheses are
$$
\begin{cases}
H_0: \mu_1 = \mu_2 = \mu_3 = \mu_4 = \mu_5 \\
H_a: \text{at least one mean price differs.}
\end{cases}
$$
The null hypothesis states that the mean price is the same across all cut categories. The alternative hypothesis states that at least one cut category has a different mean price.
...
The output reports the degrees of freedom, the $F$ statistic, and the corresponding *p*-value. Because the *p*-value is below $\alpha = 0.05$, we reject $H_0$ and conclude that mean price differs across at least one cut category. This conclusion refers to an unadjusted comparison across cut groups. It does not imply that cut quality causes differences in price, because this simple ANOVA does not adjust for other important predictors such as carat, clarity, color, or depth.

ANOVA assumes independent observations, approximately normal residuals within groups, and roughly equal variances across groups. With large datasets, the test can be robust to moderate departures from normality, but strong skewness or unequal variances should still be considered when interpreting the result.

Rejecting $H_0$ tells us that not all group means are equal, but it does not identify which cut categories differ from each other. Pairwise follow-up comparisons can be used for that purpose, but they are not needed for the main aim of this section, which is to introduce the overall ANOVA test.

As with other hypothesis tests, statistical significance should be interpreted together with practical relevance. The `diamonds` dataset is large, so even small differences in mean price may produce very small *p*-values. For pricing or inventory decisions, the magnitude of the differences and their relevance in context matter as much as the rejection of the null hypothesis.
...
Suppose we examine two numerical variables and observe that larger values of one variable tend to occur with larger values of another. For example, in sales data, higher advertising expenditure may be associated with higher product sales. In exploratory analysis (see Section [-@sec-EDA-sec-multivariate]), scatter plots and correlation matrices were used to assess such relationships visually. A correlation test provides a formal way to evaluate whether the observed linear association is stronger than would be expected under the hypothesis of no population correlation.

The Pearson correlation coefficient measures the strength and direction of a linear association between two numerical variables. Its population value is denoted by $\rho$, and its sample estimate is usually denoted by $r$. Values of $r$ close to 1 indicate a strong positive linear association, values close to -1 indicate a strong negative linear association, and values near 0 indicate little linear association. Pearson correlation does not measure every kind of relationship: nonlinear patterns may be poorly summarized by a single correlation coefficient.

To illustrate, we examine the relationship between `carat` and `price` in the `diamonds` dataset from the **ggplot2** package. Larger diamonds are generally more expensive, so we expect a positive association. We begin with a scatter plot, because the visual pattern is essential for interpreting the correlation coefficient:

```{r}
ggplot(diamonds, aes(x = carat, y = price)) +
  geom_point(alpha = 0.3, size = 0.6) +
  labs(x = "Diamond Weight (Carats)", y = "Price (dollars)")
```

The scatter plot shows a clear upward trend: diamonds with larger carat values tend to have higher prices. The relationship is positive, although the spread also increases for larger diamonds. This visual pattern is important because a correlation coefficient summarizes only the overall linear tendency and does not describe the full shape or variability of the relationship.
...
```{r}
cor_test = cor.test(diamonds$carat, diamonds$price, method = "pearson")
cor_test
```

The output reports the sample correlation coefficient, the *p*-value, and a confidence interval for the population correlation. The estimated correlation is `r round(cor_test$estimate, 2)`, indicating a strong positive linear association between carat and price. The 95 percent confidence interval is (`r round(cor_test$conf.int[1], 3)`, `r round(cor_test$conf.int[2], 3)`), which provides a plausible range for the population correlation.

The *p*-value is very small, so we reject $H_0$ at $\alpha = 0.05$. However, in a large dataset such as `diamonds`, statistical significance is not surprising: even weak correlations can produce very small *p*-values when the sample size is large. The more informative quantities here are the estimated correlation, the confidence interval, and the scatter plot. Together, they show that `carat` and `price` have a strong positive linear association in this dataset.

This result should be interpreted carefully. A strong correlation does not imply causation, and the correlation test does not adjust for other variables. Diamond price is also related to other characteristics, such as cut, clarity, color, and depth. Moreover, Pearson correlation summarizes linear association only; if the relationship is curved or changes across the range of the data, a single correlation coefficient may hide important structure. Multivariate regression models, introduced in Chapter [-@sec-ch10-regression], provide a more appropriate framework for studying several predictors jointly.
