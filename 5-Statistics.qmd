...
The computed interval is (`r round(ci_lower, 2)`, `r round(ci_upper, 2)`), which means we are 95% confident that the true average number of customer service calls for churned customers lies within this range. More precisely, if we were to draw many random samples and construct a confidence interval from each, approximately 95% of those intervals would capture the true population mean.
...
```{r fig-hypothesis-testing, echo = FALSE, out.width = "75%", fig.cap = "Visual summary of hypothesis testing, showing how sample evidence informs the decision to reject or do not reject the null hypothesis ($H_0$)."}
...
The *95% confidence interval* is (`r round(t_test$conf.int, 2)`). Since this interval does not include 2, the result reinforces our conclusion. The sample mean is `r round(t_test$estimate, 2)`, which serves as our best estimate of the true average in the population.
...
The output includes three key results: the *p*-value, the confidence interval, and the observed sample proportion. The *p*-value is `r round(prop_test$p.value, 3)`. Because it exceeds the standard significance level of $\alpha = 0.05$, we do not reject the null hypothesis. We conclude that there is no statistically significant evidence to suggest that the population proportion of churners differs from 15%.

The *95% confidence interval* for the true proportion is (`r round(prop_test$conf.int, 3)`). Since this interval contains 0.15, the result is consistent with the decision not to reject $H_0$. The *observed sample proportion* is `r round(prop_test$estimate, 3)`, which serves as our best estimate of the true churn rate in the population.
:::
...
ggplot(data = churn) +
  ...
  labs(
    #title = "Boxplot of International Calls by Churn",
    x = "Churn",
    y = "International Calls"
  )

ggplot(data = churn) +
  geom_density(aes(x = intl.calls, fill = churn)) +
  labs(
    #title = "Distribution of International Calls by Churn",
...
This function outputs the test statistic, *p*-value, degrees of freedom, confidence interval, and estimated group means. The *p*-value is `r round(t_test_calls$p.value, 4)`, which is less than the standard significance level of $\alpha = 0.05$. We therefore reject the null hypothesis and conclude that the mean number of international calls differs between churners and non-churners.

The *95% confidence interval* for the difference in means is (`r round(t_test_calls$conf.int, 3)`). Because zero is not in this range, the result supports our conclusion that the group means are different. The estimated group means are `r round(t_test_calls$estimate[1], 2)` for churners and `r round(t_test_calls$estimate[2], 2)` for non-churners. These values suggest that churners tend to make fewer international calls on average, an insight that may have business implications.
...
ggplot(data = churn) + 
  ...
  labs(
    #title = "Customer Churn by Voice Plan",
    x = "Voice Plan",
...
ggplot(data = churn) + 
  ...
  labs(
    #title = "Proportion of Customers Who Churned by Voice Plan",
    x = "Voice Plan",
    y = "Proportion",
    fill = "Churn"
  ) 
...
The *95% confidence interval* for the difference in proportions is (`r round(z_test$conf.int, 3)`). Because this interval does not include zero, it supports our conclusion that there is a meaningful difference between the groups. The observed sample proportions of churners are `r round(z_test$estimate[1], 3)` for customers with a Voice Mail Plan and `r round(z_test$estimate[2], 3)` for customers without one, indicating that customers without a Voice Mail Plan are more likely to churn.
...
The `chisq.test()` function compares the observed counts in the table with the expected counts under the assumption of independence. It returns the Chi-square statistic, degrees of freedom, expected frequencies, and a corresponding *p*-value.

The Chi-square test relies on large-sample approximations, meaning that its results are valid only when the expected cell counts are sufficiently large. As a general guideline, each expected frequency should be at least 5. If several cells contain smaller expected counts, the Chi-square approximation to the test statistic may become unreliable. In such cases, especially with small samples or sparse contingency tables, alternatives such as Fisher’s exact test are more appropriate.

To inspect the expected cell counts directly, we can use:

```{r}
chisq_test$expected
```

This output shows how many observations would be expected in each cell if the variables were independent. Comparing these expected counts with the observed ones helps identify which groups contribute most to the association.

In this example, because the *p*-value is less than the significance level ($\alpha = 0.05$), we reject the null hypothesis and conclude that marital status and deposit purchase are associated rather than independent.

The expected counts are particularly informative. They show how many deposit purchases would be expected in each marital category if no association existed. By comparing these to the observed counts, we can identify which group differences are driving the result. For instance, if married customers subscribe to deposits more frequently than expected, they may be a key contributor to the observed association.
...
The Chi-square test thus provides a rigorous foundation for uncovering associations between categorical variables. It complements visualization-based insights and supports data-driven decision-making in customer behavior analysis.
...
The output provides three key results. First, the *p*-value is close to zero and therefore well below the significance level ($\alpha = 0.05$). Thus, we reject the null hypothesis and conclude that the correlation between carat and price is statistically significant. Second, the correlation coefficient is `r round(cor_test$estimate, 2)`, reflecting a strong positive linear association. Finally, the 95% confidence interval for the true population correlation is (`r round(cor_test$conf.int, 3)`), which does not include zero and thus reinforces the evidence for a significant relationship.
