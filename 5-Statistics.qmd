...
```{r fig-stat-inference-pillars, echo=FALSE, out.width="95%", fig.cap="The three core goals of statistical inference: point estimation, confidence intervals, and hypothesis testing. Together they support reliable generalisation from sample data."}
...
The average annual transaction amount among churners is `r round(mean(churned_customers$transaction.amount.12), 2)`. This sample mean serves as a point estimate of the corresponding population mean.
:::
...
In Section [-@sec-EDA-sec-numeric], we examined the distribution of the total credit limit (`credit.limit`) for churners and non-churners using violin and histogram plots. These visualisations suggested that churners may have slightly lower credit limits. The next step is to assess whether this difference is statistically significant.
...
> *Practice:* Test whether the average tenure (`months.on.book`) differs between churners and non-churners using `t.test(months.on.book ~ churn, data = churnCredit)`. Visualisations for this variable appear in Section [-@sec-EDA-sec-numeric].
...
> *Practice:* Test whether education level is associated with churn in the *churnCredit* dataset. Follow the same steps as above. For more information on the `education` variable, see Section [-@sec-EDA-categorical] in Chapter [-@sec-ch4-EDA].
