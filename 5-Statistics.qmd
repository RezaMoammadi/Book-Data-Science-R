...
The function returns several key results: a test statistic, *p*-value, confidence interval, and degrees of freedom. The *p*-value is `r round(t_test$p.value, 3)`, which is less than $\alpha = 0.05$. We therefore reject the null hypothesis and conclude that the average number of calls differs from 2.
...
The *p*-value is `r round(z_test$p.value, 4)`, which is less than the significance level of 0.05. This leads us to reject the null hypothesis and conclude that the proportion of Voice Mail Plan users differs significantly between churners and non-churners.
...
The output includes three key results. First, the *p*-value is `r round(cor_test$p.value, 4)`, which is less than the significance level ($\alpha = 0.05$). This leads us to reject the null hypothesis, concluding that the correlation between carat and price is statistically significant. Second, the correlation coefficient is `r round(cor_test$estimate, 2)`, indicating a strong positive linear association. Finally, the 95% confidence interval for the true population correlation is `r round(cor_test$conf.int, 2)`, which does not include zero, further reinforcing the statistical significance of the relationship.
