...
Preprocessing steps such as imputation depend on the data distribution, so they should be applied after partitioning to reduce the risk of data leakage (see Section [-@sec-ch6-data-leakage]). In this case study, we apply the same random imputation strategy separately within each subset after splitting.
...
> *Practice:* Repartition the `churn` dataset into a 70% training set and a 30% test set. Apply the same imputation strategy to each subset after splitting and verify that the class distribution of `churn` is preserved across both sets.
