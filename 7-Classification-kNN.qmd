...
It is important to note that in this illustration, the test set is used to compare different values of $k$. Strictly speaking, this approach can introduce optimistic bias, because information from the test data influences hyperparameter selection. This represents a form of information leakage during model selection, because the test data influence the tuning process (see Section [-@sec-ch6-data-leakage]).

A more principled strategy is to use cross-validation within the training set to choose $k$, and then evaluate the final model on a separate test set (see Section [-@sec-ch6-cross-validation]). In practice, this may involve splitting the data into three subsets: a training set for fitting the model, a validation set for tuning $k$, and a test set for final evaluation. Alternatively, cross-validation within the training data provides a more stable and lower-variance basis for hyperparameter tuning.
