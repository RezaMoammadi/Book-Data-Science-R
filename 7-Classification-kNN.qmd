...
```{r fig-ch7-ex-drug-1, echo = FALSE, out.width = "90%", fig.cap = "Scatter plot of Age vs. Sodium/Potassium Ratio for 200 patients, with drug type indicated by color and shape."}
...
```{r fig-ch7-ex-drug-2, echo = FALSE, out.width = "90%", fig.cap = "Scatter plot of age versus sodium/potassium ratio for 200 patients, with drug type shown by color and shape. The three new patients are highlighted with large orange circles."}
...
p + geom_point(data = test_set, aes(x = age, y = ratio), colour = "#E66101", size = size, shape = 1) +
    geom_point(data = test_set, aes(x = age, y = ratio), colour = "#E66101", size = 1   , shape = 8)
```

For Patient 1, who lies deep within a cluster of green-circle points (Drug A), the classification is straightforward. All nearest neighbors belong to Drug A, making the prediction confident and unambiguous.
...
p + coord_cartesian(xlim = c(x - r_x, x + r_x), ylim = c(y - r_y, y + r_y)) +
    geom_point(aes(x = x, y = y), colour = "#E66101", size = size, shape = 1) +
    geom_point(aes(x = x, y = y), colour = "#E66101", size = 4, shape = 8) 
