...
### How Classification Works {.unnumbered .unlisted}
...
### Classification Algorithms and the Role of kNN {.unnumbered .unlisted}
...
-   Decision Trees and Random Forests capture nonlinear relationships and feature interactions (see Chapter [-@sec-ch12-tree-models]).

-   Neural Networks are high-capacity models designed for complex or unstructured data (see Chapter [-@sec-ch13-neural-networks]).
...
The appropriate encoding strategy depends on whether a variable is *binary*, *nominal*, or *ordinal*. These techniques were detailed in Section [-@sec-ch6-encoding].
...
Two widely used scaling methods address this issue: *min–max scaling* and *z-score scaling* (introduced in Section [-@sec-ch6-feature-scaling]). Min–max scaling rescales values to a fixed range, typically $[0, 1]$, ensuring that all features contribute on the same numerical scale. Z-score scaling centers features at zero and scales them by their standard deviation, making it preferable when features have different units or contain outliers.
...
To determine a suitable value of $k$, it is common to evaluate a range of options using a validation set or cross-validation. Importantly, hyperparameter selection should not be performed on the final test set, as this would allow information from the test data to influence model selection and lead to optimistic bias (see Section [-@sec-ch6-data-leakage]). Instead, $k$ should be chosen using only the training data, either by splitting it into training and validation subsets or by applying cross-validation (see Section [-@sec-ch6-cross-validation]).

Performance metrics such as accuracy, precision, recall, and the F1-score can guide this choice. These metrics are discussed in detail in Chapter [-@sec-ch8-evaluation]. For simplicity, we focus here on *accuracy* (also called the success rate), which measures the proportion of correct predictions.

As an example, Figure [-@fig-ch7-kNN-plot] presents the accuracy of the kNN classifier for $k$ values ranging from 1 to 20. Accuracy fluctuates as $k$ increases, with the best performance achieved at $k = 7$, where the algorithm reaches its highest validation accuracy.
...
The `partition()` function performs a random split while preserving the class distribution of the target variable. Readers may verify that the churn rate is similar across both sets (see Section [-@sec-ch6-cross-validation]).
...
> *Practice:* For the 70%–30% split, apply the same imputation strategy using training-derived modes and confirm that no missing values remain in `education`, `income`, and `marital`.
...
Since the `income` feature is ordinal, we follow the guidance in Section [-@sec-ch6-encoding] and assign representative numeric values that approximate the economic magnitude of each income range. Specifically, we encode `"<40K"` = 20, `"40K-60K"` = 50, `"60K-80K"` = 70, `"80K-120K"` = 100, and `">120K"` = 140, for both the training and test sets:
...
By performing hyperparameter selection using only the training data, we prevent information from the test set from influencing the tuning process. This avoids optimistic bias and preserves the integrity of the final model evaluation (see Section [-@sec-ch6-data-leakage]).
...
In the chapters that follow, we turn to more advanced classification methods, beginning with Naive Bayes (Chapter [-@sec-ch9-bayes]), followed by Logistic Regression (Chapter [-@sec-ch10-regression]) and Decision Trees (Chapter [-@sec-ch12-tree-models]). These models address many of the practical limitations encountered with kNN and provide more scalable and flexible tools for real-world predictive modeling.
...
#### Hands-On Practice: Applying kNN to the `bank` Dataset {.unnumbered .unlisted}
...
##### Data Exploration and Preparation {.unnumbered .unlisted}

11. Load the `bank` dataset and display its structure. Identify the target variable and the predictor variables.

12. Perform an initial EDA: What are the distributions of key numeric variables such as `age`, `balance`, and `duration`? Are there any unusually high or low values that might influence distance calculations in kNN?

13. Explore potential associations: Are there noticeable differences in numeric features, such as `balance` and `duration`, between customers who subscribed to a deposit and those who did not? Are there categorical features, such as `job` and `marital`, that appear to be associated with the outcome?
...
##### Diagnosing the Impact of Preprocessing {.unnumbered .unlisted}
...
##### Choosing the Optimal k {.unnumbered .unlisted}
...
##### Building and Evaluating the kNN Model {.unnumbered .unlisted}
...
#### Critical Thinking and Real-World Applications {.unnumbered .unlisted}
