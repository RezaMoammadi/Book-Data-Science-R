...
As discussed more generally in Chapter [-@sec-ch6-setup-data], different types of predictors often require different preprocessing steps before modeling. For kNN, these choices are especially consequential because similarity can be computed meaningfully only when predictors are represented numerically and placed on comparable scales.
