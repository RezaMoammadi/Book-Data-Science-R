...
Figure [-@fig-ch11-simple-tree] illustrates a classification tree trained on the `loan` dataset introduced in Chapter [-@sec-ch9-bayes]. The model predicts whether a loan application would be `approved` or `rejected` based on features such as `cibil_score`, `loan_term`, `loan_amount`, and `income_annum`. Internal nodes represent splitting rules, and terminal nodes (leaves) provide the predicted class together with class probabilities.

```{r}
#| label: fig-ch11-simple-tree
#| echo: false
#| out.width: "100%"
#| fig-cap: A classification tree built using the CART algorithm on the loan dataset to predict whether a loan application would be approved or rejected based on features such as cibil_score, loan_term, loan_amount, and income_annum. Terminal nodes display predicted class and class probabilities, illustrating CART’s rule-based structure.
...
tree.cart = rpart(formula, data = loan, method = "class")
...
### Making Predictions with a Decision Tree {.unnumbered .unlisted}
...
### Advantages and Limitations {.unnumbered .unlisted}
...
### Strengths and Limitations of Random Forests {.unnumbered .unlisted}
...
#### Conceptual Questions (Core) {.unnumbered .unlisted}
...
#### Hands-On Practice (Applied): Classification with the `churn_mlc` Dataset {.unnumbered .unlisted}

In Section [-@sec-ch11-case-study], we fitted logistic regression, Naive Bayes, and k-Nearest Neighbors models to `churn_mlc` using the Data Science Workflow. In this set of exercises, we extend the analysis by fitting tree-based models and comparing their performance to earlier methods. You can reuse your earlier data preparation and partitioning code.

##### Data Setup for Modeling {.unnumbered .unlisted}

13. Load `churn_mlc`, inspect its structure, and identify the response variable and candidate predictors.

14. Partition the dataset into a training set (80%) and a test set (20%) using the `partition()` function from the **liver** package. Use the same random seed as in Section [-@sec-ch12-case-study].
...
##### Modeling with CART {.unnumbered .unlisted}
...
##### Modeling with C5.0 {.unnumbered .unlisted}
...
##### Modeling with Random Forests {.unnumbered .unlisted}
...
#### Hands-On Practice (Applied): Regression Trees and Random Forests with the `red_wines` Dataset {.unnumbered .unlisted}
...
##### Modeling and Evaluation {.unnumbered .unlisted}
...
#### Hands-On Practice (Challenge): High-Dimensional Classification with the `caravan` Dataset {.unnumbered .unlisted}
...
##### Random Forest Classification and Tuning {.unnumbered .unlisted}
