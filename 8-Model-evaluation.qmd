...
To illustrate these ideas, we return to the kNN model from Section [-@sec-ch8-confusion-matrix], which predicts customer churn (`churn = yes`). By specifying `type = "prob"` in the `kNN()` function, we obtain predicted probabilities rather than hard class labels:
