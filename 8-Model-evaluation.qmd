...
### Why Is Model Evaluation Important? {.unnumbered .unlisted}
...
To see this in practice, we revisit the kNN model used in the churn case study of the previous chapter (Section [-@sec-ch7-knn-churn]). The following code computes the confusion matrix for the test set:
...
> *Practice:* Repeat the kNN classification from Section [-@sec-ch7-knn-churn] using $k = 2$ and compute the corresponding confusion matrix. Using that confusion matrix, calculate the sensitivity and compare it with the value obtained earlier for $k = 7$. How does changing $k$ affect the model’s ability to identify churners, and what might explain the difference?
...
> *Practice:* Repeat the kNN classification from Section [-@sec-ch7-knn-churn] using $k = 2$ and compute the corresponding confusion matrix. Using that confusion matrix, calculate the specificity and compare it with the value obtained earlier for $k = 7$. How does changing $k$ affect the model’s ability to correctly identify non-churners? What does this reveal about the relationship between $k$ and false positive predictions?
...
> *Practice:* Repeat the kNN classification from Section [-@sec-ch7-knn-churn] using $k = 2$. Compute the resulting confusion matrix and calculate the precision, recall, and F1-score. How do these values compare with those for $k = 7$? Which metrics increase, which decrease, and what does this reveal about the effect of $k$ on model behavior?
...
```{r fig-ch8-confusion-matrices, echo = FALSE, out.width = "70%", fig.cap = "Confusion matrices for binary (left) and multi-class (right) classification. Diagonal cells show correct predictions; off-diagonal cells show misclassifications. Matrix size grows with the number of classes."}
...
#### Hands-On Practice: Model Evaluation with the `bank` Dataset {.unnumbered .unlisted}
...
##### Model Training and Evaluation {.unnumbered .unlisted}
