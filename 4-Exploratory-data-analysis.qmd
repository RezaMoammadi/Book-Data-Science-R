...
ggplot(data = churn) +
  geom_density(aes(x = day.mins, fill = churn)) +
...
ggplot(data = churn) +
  geom_density(aes(x = eve.mins, fill = churn)) +
...
ggplot(data = churn) +
  geom_boxplot(aes(x = churn, y = night.mins), 
               fill = c("#F4A582", "#A8D5BA")) +
  labs(x = "Churn", y = "Nighttime Minutes")

ggplot(data = churn) +
  geom_density(aes(x = night.mins, fill = churn)) +
...
ggcorrplot(cor_matrix, type = "lower", lab = TRUE, lab_size = 2, tl.cex = 8, 
...
ggplot(data = churn) +
    ...
    geom_abline(intercept = 400, slope = -0.6) + 
...
ggplot(data = sub_churn, aes(x = churn, label = scales::percent(prop.table(after_stat(count))))) +
    geom_bar(fill = c("#F4A582", "#A8D5BA")) + 
    geom_text(stat = 'count', vjust = 0.4, size = 8)
```
