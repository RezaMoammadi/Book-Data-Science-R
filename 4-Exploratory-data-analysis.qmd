...
The central scatter plot shows a clear positive association: customers who spend more also tend to make more transactions. Most observations fall within a broad diagonal region of moderate spending and transaction counts, where churners and non-churners largely overlap. The marginal histograms at the top and right of the figure summarise the distributions of `transaction.amount.12` and `transaction.count.12` separately, and they provide a quick visual comparison of usage levels between churned and active accounts.

> *Try it yourself:* Replace `type = "histogram"` with `type = "density"` in the `ggMarginal()` function to add marginal density curves instead of histograms. Compare the two versions and consider which one better highlights the distribution of each variable.
