...
As an alternative, a pie chart provides a compact visualisation of the same information. The following code produces a clear and reader-friendly version:

```{r out.width = "50%"}
pie_data <- churnCredit |>
  dplyr::count(churn) |>
  dplyr::mutate(prop = n / sum(n))

ggplot(pie_data, aes(x = "", y = prop, fill = churn)) +
  geom_col(width = 1) +
  coord_polar(theta = "y") +
...
Pie charts are less informative for comparing multiple groups, but they can be effective when visualising the proportions of a single binary variable.

The earlier bar chart builds on basic plotting techniques introduced in Section [-@sec-ch1-visualization]. A simpler version, without colour or percentage labels, can be produced with:
...
The simpler plot offers a quick overview, whereas the enhanced version provides clearer communication by emphasising proportions. Such refinements are valuable when presenting results to non-technical stakeholders, as they make relative differences immediately visible.

Class imbalance is not just a descriptive feature; it has practical implications for modelling. Predictive algorithms trained on imbalanced data tend to favour the majority class, leading to biased predictions and lower sensitivity to minority outcomes. This issue will be revisited in Chapter [-@sec-ch6-setup-data], Section [-@sec-ch6-balancing], where methods for addressing imbalance are introduced.

Having established the overall distribution of the target variable, the next step is to explore how other categorical features relate to churn. These variables may reveal distinct customer segments or behavioural patterns associated with higher attrition risk.
