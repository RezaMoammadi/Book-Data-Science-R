...
if(knitr::is_latex_output()){
  ...
  ggplot_html = ggplot_1 +
    ...
    theme(title = element_text(size = 14, face = "bold"),
          ...
          plot.title = element_text(hjust = 0.8, vjust = -18, color = "#eddee4", size = 60, 
                                  face = "bold.italic")) + 
          scale_color_brewer(palette = "Set2")
...
### Overview of the *churn* Dataset {.unnumbered}
...
### Relationship Between Subscription Plans and Churn {.unnumbered}
...
ggplot(data = churn) + 
  ...
  scale_fill_manual(values = c("#F4A582", "#A8D5BA"))  
...
### Relationship Between Voice Mail Plan and Churn {.unnumbered}
...
```{r out.width = "60%"}
ggplot(data = churn) +
  geom_bar(aes(x = factor(customer.calls)), fill = "#80CDC1", color = "black") +
...
ggplot(data = churn) +
  geom_density(aes(x = day.mins, fill = churn), alpha = 0.5) +
  scale_fill_manual(values = c("#F4A582", "#A8D5BA")) +
...
ggplot(data = churn, aes(x = churn, y = eve.mins)) +
  ...
  geom_boxplot(width = 0.3, fill = c("#F4A582", "#A8D5BA")) +
  labs(x = "Churn", y = "Evening Minutes")

ggplot(data = churn) +
  geom_density(aes(x = eve.mins, fill = churn), alpha = 0.5) +
  scale_fill_manual(values = c("#F4A582", "#A8D5BA")) +
...
ggplot(data = churn) +
  geom_boxplot(aes(x = churn, y = night.mins), fill = c("#F4A582", "#A8D5BA")) +
  labs(x = "Churn", y = "Nighttime Minutes")

ggplot(data = churn) +
  geom_density(aes(x = night.mins, fill = churn), alpha = 0.5) +
  scale_fill_manual(values = c("#F4A582", "#A8D5BA")) +
...
ggplot(data = churn) +
  geom_density(aes(x = intl.calls, fill = churn), alpha = 0.5) +
  scale_fill_manual(values = c("#F4A582", "#A8D5BA")) +
...
ggcorrplot(cor_matrix, type = "upper", lab = TRUE, lab_size = 2, tl.cex = 8, 
           colors = c("#6D9EC1", "white", "#E46726"),
           title = "Visualization of the Correlation Matrix")
```

The resulting heatmap highlights two main patterns. First, the *charge variables* (such as `day.charge`, `eve.charge`, and `intl.charge`) are perfectly correlated with their corresponding *minutes variables*. This is expected, as charges are calculated directly from call duration. Including both in a model would add redundancy without new information.
...
ggplot(data = churn) +
    ...
    geom_abline(intercept = 400, slope = -0.6, color = "#377EB8", linewidth = 1) + 
    scale_color_manual(values = c("#F4A582", "#A8D5BA")) + 
    labs(x = "Evening Minutes", y = "Daytime Minutes", color = "Churn")
```
...
ggplot(data = churn, aes(x = day.mins, y = customer.calls, color = churn)) +
  geom_point(size = 1) +
  geom_point(data = data.frame(day.mins = 110, customer.calls = 5.5), colour = "#377EB8", size = 50, shape = 1) + 
  geom_point(data = data.frame(day.mins = 310, customer.calls = 2.0), colour = "#377EB8", size = 40, shape = 1) +
  scale_color_manual(values = c("#F4A582", "#A8D5BA")) +
  labs(x = "Daytime Minutes", y = "Customer Service Calls", color = "Churn")
```
...
The following exercises reinforce the core principles of exploratory data analysis (EDA) introduced in this chapter. They combine conceptual understanding, hands-on implementation, and higher-level synthesis tasks. Begin with conceptual questions to strengthen foundational ideas, proceed with applied EDA using the *bank* and *churnTel* datasets, and conclude with challenge problems that emphasize insight generation and reflection.

### Conceptual Questions {.unnumbered}

1. Why is exploratory data analysis (EDA) essential before modeling? What risks might arise if this step is skipped?

2. If a variable shows no clear relationship with the target during EDA, should it be excluded from modeling? Consider possible interactions or hidden effects.
...
4. How can you detect and address correlated predictors during EDA? Why is this important for model stability and interpretability?

5. What happens when highly correlated variables are included in a predictive model? Discuss effects on accuracy and interpretability.

6. Is it always appropriate to remove one of two correlated predictors? When might keeping both be justified?

7. For each method—histograms, box plots, density plots, scatter plots, summary statistics, correlation matrices, contingency tables, bar plots, and heatmaps—indicate whether it applies to categorical, numerical, or both data types.

8. A telecommunications firm finds that customers with high day and evening minutes tend to churn more. What actions could the company take in response?

9. Suppose several variable pairs show high correlation ($r > 0.9$). How would you handle this to ensure a robust model?

10. Why should both statistical and practical significance be considered when evaluating correlations? Provide an example of a strong but practically unimportant relationship.

11. Why is it important to explore multivariate relationships in EDA? Give an example of an interaction that would be missed by univariate analysis.

12. How do visualizations enhance EDA? Provide two examples where visual tools reveal insights that summary statistics cannot.

13. Suppose customers with both high daytime usage and frequent service calls are more likely to churn. What business strategies could be informed by this insight?

14. What causes outliers in data, and how can you decide whether to retain, modify, or exclude them?

15. Why must missing values be addressed during EDA? Compare strategies for handling missing data and explain when each is appropriate.

### Hands-On Practice: Exploring the *bank* Dataset {.unnumbered}

The *bank* dataset from the **liver** package contains data from direct marketing campaigns of a Portuguese bank. The objective is to predict whether a client subscribes to a term deposit (`deposit = "yes"` or `"no"`). This dataset will reappear in later chapters on classification (Chapter [-@sec-ch7-classification-knn]) and neural networks (Chapter [-@sec-ch12-neural-networks]). For more details about this dataset see  <https://rdrr.io/cran/liver/man/bank.html>.
...
16. Summarize the structure, number of variables, and their types. What does this reveal about the dataset?

17. Visualize the target variable `deposit`. What proportion of clients subscribed to a term deposit?

18. Explore the binary variables `default`, `housing`, and `loan` using bar plots and contingency tables. What patterns do you observe?

19. Visualize the distributions of numerical features using histograms and box plots. Note any skewness or unusual observations.

20. Identify outliers in the numerical variables. What strategies would you consider for handling them?

21. Compute and visualize the correlation matrix. Which variables show strong correlations, and how might you address them before modeling?

22. Summarize your main EDA findings. How would you present these results in a report for a non-technical audience?

23. Translate your findings into business insights. What actions could the bank take based on these patterns?

24. Investigate whether higher values of `campaign` (number of contacts) are associated with greater subscription rates. Visualize and interpret your findings.

25. Propose one new derived feature that could improve model performance based on your observations.

26. Examine subscription rates by `month`. Do certain months show higher success rates, and what might explain this pattern?

27. Explore the relationship between `job` and `deposit`. Which occupational groups have the highest likelihood of subscription?

28. Analyze the combined effect of `education` and `job` on subscription outcomes. What patterns emerge from this interaction?

29. Examine whether the `duration` of the last contact influences the probability of a term deposit.

30. Compare campaign success rates across different contact frequencies. What strategies might the bank adopt to improve future campaign effectiveness?

### Hands-On Practice: Exploring the *churnTel* Dataset {.unnumbered}

The *churnTel* dataset from the **liver** package contains information on 7,043 telecommunications customers and 21 features describing their demographic, contract, and service attributes. The goal is to explore the factors related to customer churn (`churn = "yes"` or `"no"`) and identify patterns that may inform retention strategies.
...
31. Examine the structure of the dataset. Identify the target variable and summarize the main feature types.

32. Visualize the distribution of the target variable `churn`. What proportion of customers have left the service?

33. Explore categorical variables such as `contract`, `internet.service`, and `payment.method` using bar plots. What trends or imbalances do you observe?

34. Examine numerical variables such as `tenure`, `monthly.charge`, and `total.charges` using histograms and box plots. Identify any skewed distributions or outliers.

35. Analyze the relationship between `tenure` and `churn`. Do long-term customers exhibit lower churn rates?

36. Investigate how `contract` type interacts with churn. Which contract types are associated with higher retention?

37. Compute and visualize correlations among numerical variables. Which pairs show the strongest relationships, and how might this influence model design?

38. Summarize your main findings from the EDA. What patterns stand out, and how might they guide future predictive modeling or business decisions?

### Challenge Problems {.unnumbered}

39. Create a concise, two-plot visual summary that communicates a key insight from your EDA of the *bank* dataset. Focus on clear labeling and a brief caption suitable for presentation to a non-technical audience.

40. Using the *adult* dataset, identify a subgroup of individuals likely to earn over $50K. Describe their demographic and occupational profile and explain how you uncovered it through EDA.

41. A variable appears weakly related to the target in univariate plots. Under what conditions could it still enhance model accuracy? Discuss how this might occur.

42. Examine whether the proportion of `deposit` outcomes differs by `marital` status or `job` category. What insights or hypotheses emerge from this subgroup analysis?

43. Using the *adult* dataset, identify predictors that may contribute little to model performance. Justify your choices using EDA evidence and discuss how pruning such features could improve interpretability and efficiency.
