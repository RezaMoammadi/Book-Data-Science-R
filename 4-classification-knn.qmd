...
Here we use mode imputation for these categorical variables. Although simple, this approach is transparent and sufficient for illustrating the preprocessing workflow. Rather than calculating and applying the modes manually, we use the **recipes** package to define the imputation procedure. This makes it easier to ensure that the imputation values are learned only from the training data and then applied unchanged to the test data.
...
# Define the imputation procedure
imputation_recipe = recipe(churn ~ ., data = train_set) |>
  step_impute_mode(education, income, marital)

# Learn the imputation values from the training data
imputation_prep = prep(imputation_recipe, training = train_set)

# Apply the learned procedure
train_set = bake(imputation_prep, new_data = NULL, composition = "data.frame")
test_set = bake(imputation_prep, new_data = test_set, composition = "data.frame")

train_set = droplevels(train_set)
test_set = droplevels(test_set)
```

The `recipe()` function defines the preprocessing procedure, while `step_impute_mode()` specifies that missing values in `education`, `income`, and `marital` should be replaced by the most frequent observed category. The call to `prep()` estimates these modes using only the training data. The prepared recipe is then applied to the training data and, without re-estimating the imputation values, to the test data using `bake()`.

This distinction is important for avoiding data leakage. Information from the test set should not influence preprocessing decisions made during model development. The final calls to `droplevels()` remove factor levels that are no longer observed after replacing `"unknown"` and completing the imputation.

Mode imputation is only one possible approach. More flexible methods can use information from other predictors when estimating missing values. For example, the **recipes** package also provides k-nearest-neighbor imputation through `step_impute_knn()` and bagged-tree imputation through `step_impute_bag()`. Such methods can better reflect relationships among variables, but they also introduce additional modeling choices. For this introductory example, we retain mode imputation because it is simple, transparent, and sufficient for illustrating leakage-free preprocessing.

> *Practice:* Repeat this imputation procedure after creating a 70%–30% train–test split. Confirm that no missing values remain in `education`, `income`, and `marital`, and identify the mode learned from the training data for each variable.
...
The resulting plot shows how validation accuracy changes with $k$. In this case, the highest accuracy is achieved at $k = 9$. This does not mean that $k = 9$ is universally best; it simply indicates that, under this validation-based tuning strategy, $k = 9$ performs best for this dataset. With this value selected, we can now fit the final model on the full training set and evaluate it once on the separate test set.
