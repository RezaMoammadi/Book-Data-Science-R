...
# Data Setup for Modeling {#sec-ch6-data-setup}
...
We illustrate the train-test split using R and the **liver** package. We return to the `churn` dataset introduced in Section [-@sec-ch4-EDA-churn], which will also be used in the classification chapters that follow.
...
```{r fig-cross-validation, echo = FALSE, out.width = "65%", fig.cap = "Illustration of k-fold cross-validation. The dataset is divided into $k$ folds ($k = 5$ shown). In each iteration, the model is trained on $k - 1$ folds (green) and evaluated on the remaining fold (red)."}
...
> *Practice:* The `education` variable in the `bank` dataset includes ordered levels such as `primary`, `secondary`, and `tertiary`, but also contains an `"unknown"` category. Explain why `"unknown"` does not naturally belong on the ordinal scale. Then propose a reasonable way to handle this category before applying ordinal encoding.
...
The hands-on exercises below use the `churn`, `bank`, `loan`, and `creditcard_fraud` datasets from the **liver** package.
...
20. Partition the `churn` dataset into 75% training data and 25% test data. Use a reproducible seed and report the number of observations in each subset.
...
24. Use a two-sample Z-test to examine whether the churn proportion differs between the training and test sets of `churn`.
