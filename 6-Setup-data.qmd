...
## How to Split Data for Modeling {#sec-partitioning}
...
To demonstrate a typical train-test split, we use R and the **liver** package. We will revisit the *churn* dataset from @sec-ch4-EDA-churn, where the goal is to predict customer churn using a machine learning algorithm (introduced in the next chapter). Since the dataset is bundled with the **liver** package, we begin by loading it:
...
There are several ways to partition a dataset in R. An easy and efficient approach is to use the `partition()` function from the [**liver**](https://CRAN.R-project.org/package=liver) package, which randomly splits a dataset according to a specified ratio. The example below divides the *churn* dataset into an 80% training set and a 20% test set:
...
### A Note on Cross-Validation {.unnumbered}
...
| Type of Features                            | Suggested Test (from @sec-ch5-statistics) |
|--------------------------------------|----------------------------------|
...
Suggested hypothesis tests for validating partitions, based on the type of target feature.
:::
...
Suppose the *p*-value is `r round(test_churn$p.value, 2)`. Since this value exceeds the typical significance threshold ($\alpha = 0.05$), we fail to reject $H_0$. This suggests that the observed difference in churn rates is not statistically significant, and the split appears valid with respect to the target variable.
...
Suppose the output shows that churners (`churn = "yes"`) make up only a small proportion, say around `r round(prop.table(table(train_set$churn))[1], 2)`, compared to non-churners. This imbalance may lead to a model that overlooks the very class we aim to predict.
