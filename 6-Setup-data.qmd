...
The work in the previous chapters forms the foundation for this stage. In @sec-ch2-Problem-Understanding, we defined the modeling objective. In @sec-ch3-data-preparation, we cleaned the data and handled issues such as missing values and outliers. Chapter [-@sec-ch4-EDA] guided the exploratory analysis, and Chapter [-@sec-ch5-statistics] introduced tools to test whether differences between datasets are statistically meaningful.
...
This chapter completes Step 4 of the Data Science Workflow, *Data Setup for Modeling*. We learn how to partition a dataset into training and test subsets to simulate deployment scenarios and to support fair evaluation. We then validate that the split is statistically representative and free from data leakage, using the inferential tools introduced in Chapter [-@sec-ch5-statistics]. Next, we address class imbalance using approaches such as oversampling, undersampling, and class weighting, which are often essential when the minority class is the main analytical focus. Finally, we prepare predictors for modeling by scaling numerical features (using min–max and z-score transformations) and encoding categorical variables (using ordinal, one-hot, and frequency encoding) so that they can be used effectively by common machine learning algorithms.

By mastering these tasks, we ensure that the data is not only clean but also properly structured for training machine learning models that are robust, fair, and generalizable.
