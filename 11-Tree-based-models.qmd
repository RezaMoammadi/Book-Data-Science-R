...
## Random Forests: Boosting Accuracy with an Ensemble of Trees
...
As discussed in Chapter [-@sec-ch3-data-preparation], data preparation is a foundational step in the Data Science Workflow. In this case study, we summarize the most relevant transformations applied to prepare the *adult* dataset for modeling.
...
#### Transforming Categorical Variables {.unnumbered}
...
In the ROC plot, the *black* curve represents CART, the [*red*]{style="color:red"} curve represents C5.0, and the [*green*]{style="color:green"} curve represents Random Forest. Take a moment to study the curves: Which model consistently stays closest to the top-left corner—the sweet spot for perfect classification?
...
### Hands-On: Classification with the Churn Dataset {.unnumbered}
...
### Regression Trees and Random Forests: The redWines Dataset {.unnumbered}

We now turn to regression tasks, using the *redWines* dataset from **liver** package to predict wine quality.

### Conceptual Questions: Regression Trees and Random Forests {.unnumbered}
...
### Hands-On: Regression with the redWines Dataset {.unnumbered}
