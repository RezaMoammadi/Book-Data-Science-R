...
Use the cleaned version of the `diamonds` dataset prepared in Chapter [-@sec-ch3-data-preparation].

24. Which variable is the natural response for a regression analysis in this dataset, and which variables seem like plausible predictors?

25. Use the `partition()` function introduced earlier in the book to split the cleaned `diamonds` data into training and test sets. Use 80% of the observations for training and 20% for testing.

26. Fit a simple linear regression model on the training data predicting `price` from `carat`. Store it as `simple_model_diamonds`. Interpret the estimated intercept and slope.

27. Create a scatter plot of `price` versus `carat` and add the fitted regression line. Does a straight-line relationship appear reasonable?

28. Fit a multiple regression model on the training data predicting `price` using `carat`, `cut`, `color`, and `clarity`. Store it as `full_model_diamonds`.

29. Compare the simple and multiple regression models using the residual standard error and adjusted $R^2$ on the training data. What do these measures suggest?

30. Apply stepwise regression to `full_model_diamonds` and store the result as `stepwise_model_diamonds`. Which predictors are retained in the final specification?

31. Starting from your main diamonds model, add a quadratic term for `carat`, that is, `I(carat^2)`. Does this nonlinear term appear useful?

32. Compare your main diamonds models on the test set using mean squared error (MSE). Which model performs best on unseen data?

33. Produce the standard diagnostic plots for your preferred diamonds model. Do they suggest any serious problems with non-linearity, non-constant variance, non-normal residuals, or influential observations?

34. Based on your results, which diamonds model would you recommend if the goal is prediction, and which would you recommend if the goal is interpretation? Explain your reasoning.
...
35. In the chapter, why was it important to restrict the analysis to observations with `functioning_day == "yes"` before fitting the regression models?

36. Why was a chronological 80%-20% split more appropriate than a random split for the bike demand case study?

37. What was the motivation for modeling `sqrt_bike_count` instead of `bike_count` directly? In your answer, comment on both distributional shape and model stability.

38. The stepwise model removed only `wind_speed`. What does this suggest about the relationship between model simplicity and predictive performance in this case study?

39. The full and stepwise bike-demand models had very similar diagnostics and test-set performance. What broader lesson does this give about choosing between competing regression models?

40. Compare the goals of prediction and explanation in the bike demand case study. If the audience were city planners rather than modelers, would you still prefer the same final model? Explain your reasoning.
