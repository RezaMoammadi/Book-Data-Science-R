...
```{r fig-scoter-plot-simple-reg, echo = FALSE, out.width = "80%", fig.cap = "Scatter plot of daily revenue (euros) versus daily spend (euros) for 40 observations, with the fitted least-squares regression line (orange) showing the linear relationship."}
...
$$ {#eq-sse} where $n$ is the number of observations. This quantity corresponds to the total squared length of the orange dashed lines in @fig-residual-simple-reg. Minimizing SSE ensures that the estimated regression line best fits the observed data.
...
Logistic regression is a generalized linear model specifically designed for binary outcomes. It estimates the *probability* that an observation belongs to a particular class (e.g., churn = "yes") by applying the *logit function*, which transforms a linear combination of predictors into the log-odds of the outcome: $$
...
```{r fig-scoter-plot-non-reg, echo = FALSE, out.width = "80%", fig.cap = "Scatter plot of house price ($) versus house age (years) for the house dataset, with the fitted simple linear regression line in dashed-orange and the quadratic regression curve in blue."}
...
ggplot(data = house, aes(x = house.age, y = unit.price)) +
    ...
    ggtitle("Scatter Plot with Linear and Quadratic Fits")
```
...
Our goal is to compare these models using ROC curves and AUC, which offer threshold-independent measures of classification performance, as discussed in Chapter [-@sec-ch8-evaluation]. To ensure a fair comparison, we use the same set of features and preprocessing steps for all three models. The selected features include: `account.length`, `voice.plan`, `voice.messages`, `intl.plan`, `intl.mins`, `intl.calls`, `day.mins`, `day.calls`, `eve.mins`, `eve.calls`, `night.mins`, `night.calls`, and `customer.calls`. These features capture core aspects of a customer’s usage behavior and plan characteristics, making them informative for modeling churn.
...
> *Reflection:* How might we convert these predicted probabilities into binary class labels? What threshold would you use? What happens if you change the reference level of the `churn` variable and repeat the process? Does the interpretation of the predicted probabilities change?
