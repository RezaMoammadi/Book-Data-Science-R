...
For classification problems, random partitioning is often combined with *stratification*. A stratified split aims to preserve approximately the same outcome proportions in the training and test sets. This is especially useful when one class is uncommon, since an unstratified random split may leave too few minority-class observations in one of the subsets. More generally, stratification can also be based on another categorical or numerical variable whose distribution should be approximately preserved across the resulting subsets. Stratified partitioning is illustrated later in the `adult` case study.
