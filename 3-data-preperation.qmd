...
Reliable predictive modeling requires a clear separation between model development and final evaluation. If the same observations influence both model development and performance assessment, the resulting estimate may be overly optimistic because the model has already adapted to information from those observations.

Supervised learning therefore assigns different roles to different subsets of the data. The *training set* is used to learn data-preparation rules, fit candidate models, compare alternatives, select features, and tune hyperparameters. The *test set* is reserved for final assessment and should remain outside these decisions until the preparation workflow and model have been finalized.
...
3. *Assess* final predictive performance once on the untouched test set.
...
This separation also governs data preparation: rules estimated from the observed data must be learned within the training process rather than from the full dataset. Otherwise, information from the test set may influence model development and compromise the independence of the final assessment. These principles are developed further in Section [-@sec-data-prep-data-leakage].

Data partitioning establishes the outer separation between model development and final evaluation, while resampling methods such as cross-validation support model comparison and tuning within the training data. The following subsections explain how to choose and check an appropriate train-test split and how cross-validation is used within the training set. The broader concept of *generalization*, together with underfitting, overfitting, and the relationship between model complexity and predictive performance, is developed in Section [-@sec-evaluation-generalization].

### Choosing and Checking the Train-Test Split {.unnumbered .unlisted}

The simplest way to separate model development from final evaluation is the *train-test split*, also known as the *holdout method*. The choice of split ratio depends on the size and structure of the dataset. Common choices include 70--30, 80--20, and 90--10. A larger training set provides more information for learning preparation rules and fitting models, whereas a larger test set can provide a more stable estimate of final predictive performance. There is no universally optimal ratio; both subsets should be large enough for their intended roles.

For classification problems, random partitioning is often combined with *stratification*. A stratified split aims to preserve approximately the same outcome proportions in the training and test sets. This is especially useful when one class is uncommon, since an unstratified split may leave too few minority-class observations in one of the subsets. More generally, stratification may also be based on another categorical or numerical variable whose distribution should be reasonably preserved across the resulting subsets. Stratified partitioning is illustrated later in the `adult` case study.

A standard random split is appropriate when observations can reasonably be treated as independent and there is no ordering or grouping structure that must be preserved. When such structure is present, the partitioning strategy should reflect how future predictions will actually be made.

For time-ordered data, the training set should usually contain earlier observations and the test set later observations. This preserves the temporal direction of the prediction problem and avoids using future information when evaluating a model intended to predict future outcomes. For example, in the `bike_demand` case study in Chapter [-@sec-chapter-regression], earlier time points are assigned to the training set and later time points to the test set.

Grouped or repeated-measurement data require similar care. Observations from the same individual or group should generally remain in the same subset so that closely related information does not appear in both the training and test data. The same principle applies to clustered data, such as patients within hospitals or customers within companies. Spatial data may likewise require partitions that account for geographical dependence and reflect the locations or regions for which future predictions are intended.

The train-test split should be created before any data-dependent preparation rule is estimated. Computing imputation values, determining rare-category thresholds, estimating scaling parameters, selecting features, or balancing classes before partitioning would allow information from the test set to influence model development.

Rules defined independently of the observed sample are different. For example, if dataset documentation specifies that `"?"` represents a missing value, it can be recoded consistently in both the training and test sets. The rule follows from the meaning of the data rather than from information estimated from the observed sample. More generally, externally defined coding rules, valid-value ranges, or category orderings may be applied consistently across subsets.

After partitioning, the split should be checked to confirm that the intended design has been implemented correctly. Basic checks include verifying the number and proportion of observations assigned to each subset and, when stratification is used, confirming that the intended proportions have been reasonably preserved. For grouped or repeated-measurement data, the same individual or group should not appear in both subsets, while time-based splits should preserve the required chronological separation.

These checks concern the *design* of the split rather than the detailed characteristics of the test observations. The purpose is not to make the training and test sets as similar as possible, since some differences are expected. Nor should a random split be regenerated repeatedly simply because another seed produces more similar subsets. If particular characteristics must be preserved, they should be incorporated directly into the partitioning strategy through stratification, grouping, temporal ordering, or another appropriate structure-aware approach.

### Cross-Validation Within the Training Set {.unnumbered .unlisted}

After reserving a test set, model development could rely on a single portion of the training data for validation. However, the resulting model comparisons, hyperparameter choices, and estimates of validation performance may depend strongly on which observations happen to be assigned to that validation subset. This can be particularly problematic when the training set is small, the outcome is imbalanced, or important patterns are represented by relatively few observations.

*k*-fold cross-validation reduces this dependence on a single validation split by evaluating models across several divisions of the training data. The training set is divided into $k$ non-overlapping subsets of approximately equal size, called *folds*. In each iteration, the model is fitted using $k-1$ folds and evaluated on the remaining fold, which serves as validation data. The process is repeated $k$ times so that each fold is used once for validation.

The performance values obtained from the individual folds are then averaged to provide a cross-validated basis for comparing models or tuning hyperparameters. Common choices are $k=5$ and $k=10$. Figure [-@fig-data-prep-cross-validation] illustrates the procedure for $k=5$.
...
In classification problems, the folds are often stratified so that each contains approximately the same outcome proportions. Other data structures may require different resampling strategies. Grouped cross-validation keeps related observations in the same fold, while time-series resampling preserves temporal order.
...
3. Fit candidate preparation workflows and models using the training folds, and evaluate them on the corresponding validation fold.
4. Use the cross-validation results to select the preparation workflow, model, and hyperparameters.
5. Refit the selected workflow and model using the full training set, then assess final predictive performance once on the untouched test set.

Figure [-@fig-data-prep-cross-validation-2] shows how cross-validation is nested within the training set while the test set remains separate from model development.
...
Data-dependent preparation is part of this cross-validation process. In each iteration, imputation values, category groupings, encoding structures, scaling parameters, feature transformations, feature-selection decisions, and similar rules must be learned using only the training folds and then applied to the corresponding validation fold. Estimating these rules once from the complete training set before cross-validation would allow validation observations to influence model development indirectly. The resulting risk of data leakage is discussed in Section [-@sec-data-prep-data-leakage].

After the preferred preparation workflow, model, and hyperparameters have been selected, the preparation rules are estimated again using the full training set and the final model is fitted. The fitted preparation rules are then applied to the test data before final assessment.
...
Because the fraud class is uncommon, we use stratified partitioning to help preserve its proportion in the training and test sets. The `strata` argument of the `partition()` function specifies `Class` as the variable used for stratification:
...
fraud_splits = partition(data = creditcard_fraud, ratio = c(0.7, 0.3), strata = Class)

train_fraud = fraud_splits$part1
test_fraud  = fraud_splits$part2
```

We then examine the class proportions in the full dataset and in the two resulting subsets:
...
The fraud class (`Class = 1`) remains uncommon in both the training and test sets, while stratification helps preserve approximately the same class proportions across the subsets. It is still useful to verify the resulting distributions after partitioning to confirm that both classes are adequately represented for their intended roles.
...
We use the `partition()` function from the **liver** package to assign 80% of the observations to the training set and the remaining 20% to the test set. Because `income` is the target feature and its two classes are not equally represented, we use stratified partitioning to help preserve the income proportions across the two subsets:

```{r}
set.seed(42)

splits = partition(data = adult, ratio = c(0.8, 0.2), strata = income)

adult_train = splits$part1
adult_test  = splits$part2
```

The `set.seed()` function makes the stratified random partition reproducible. The two subsets now have distinct roles: `adult_train` is used for preparation and model development, whereas `adult_test` is reserved for final assessment and should not be used to choose preparation methods, features, models, or hyperparameters.

We verify that the income proportions have been approximately preserved in the training and test sets:
...
The training and test sets retain approximately the same income proportions as the full dataset. Small differences may still occur because the required subset sizes correspond to integer numbers of observations. The purpose of this check is to confirm that the stratified partition was implemented as intended and that both outcome classes remain adequately represented for their intended roles.
...
30. Use `partition()` to create a stratified train-test split of `creditcard_fraud` using `Class`. Report and compare the class proportions in the full, training, and test data.
