...
## Data Partitioning and Cross-Validation {#sec-data-prep-cross-validation}

Reliable predictive modeling requires a clear separation between model development and final evaluation. If the same observations influence both processes, the resulting estimate of predictive performance may be overly optimistic because the model has already adapted to information from those observations.

Supervised learning therefore assigns different roles to different subsets of the data. The *training set* is used for data preparation, model fitting, model comparison, feature selection, and hyperparameter tuning. The *test set* is reserved for final assessment and should remain outside these decisions until the preparation workflow and model have been finalized.
...
2. *Use the training data* to develop the preparation workflow, compare and tune candidate models, and fit the selected model.
...
Data partitioning establishes the outer separation between model development and final evaluation. Within the training set, cross-validation provides a structured way to compare models and tune hyperparameters without using the test data. Data-dependent preparation must follow the same separation so that information from validation or test observations does not influence model development; this principle is developed in Section [-@sec-data-prep-data-leakage].

The following subsections explain how to choose an appropriate train-test split and how cross-validation is carried out within the training set. The broader concept of *generalization*, together with underfitting, overfitting, and the relationship between model complexity and predictive performance, is developed in Section [-@sec-evaluation-generalization].

### Choosing the Train-Test Split {.unnumbered .unlisted}

The simplest way to separate model development from final evaluation is the *train-test split*, also known as the *holdout method*. Common choices include 70--30, 80--20, and 90--10. A larger training set provides more information for learning preparation rules and fitting models, whereas a larger test set can provide a more stable estimate of final predictive performance. There is no universally optimal ratio; both subsets should be large enough for their intended roles.

For classification problems, random partitioning is often combined with *stratification* so that the outcome proportions are approximately preserved across the training and test sets. This is especially useful when one class is uncommon. More generally, stratification may be based on another variable whose distribution should be reasonably preserved. Stratified partitioning is illustrated later in the `adult` case study.

A standard random split is appropriate when observations can reasonably be treated as independent and there is no ordering or grouping structure that must be preserved. When observations are not independent, the partitioning strategy should preserve the structure relevant to the prediction problem.
...
For grouped or repeated-measurement data, observations from the same individual or group should generally remain in the same subset so that closely related information does not appear in both the training and test data. The same principle applies to clustered data, such as patients within hospitals or customers within companies. Spatial data may likewise require partitions that reflect geographical dependence and the locations or regions for which future predictions are intended.

The train-test split should be created before any data-dependent preparation rule is estimated. Otherwise, information from the test set may influence model development. The distinction between data-dependent rules and rules defined independently of the observed sample is developed in Section [-@sec-data-prep-data-leakage].

After partitioning, the split should be checked to confirm that the intended design has been implemented correctly. This includes verifying subset sizes and, when relevant, checking that stratification, grouping, or temporal ordering has been preserved. These checks concern the *design* of the split rather than making the training and test sets as similar as possible. A random split should therefore not be regenerated simply because another seed produces more similar subsets; required characteristics should instead be built into the partitioning strategy.

### Cross-Validation Within the Training Set {.unnumbered .unlisted}

After reserving a test set, model development could rely on a single portion of the training data for validation. However, the resulting model comparisons and hyperparameter choices may depend strongly on which observations happen to be assigned to that validation subset. This can be particularly problematic when the training set is small, the outcome is imbalanced, or important patterns are represented by relatively few observations.
...
The resulting performance values are averaged to provide a cross-validated basis for comparing models or tuning hyperparameters. Common choices are $k=5$ and $k=10$. Figure [-@fig-data-prep-cross-validation] illustrates the procedure for $k=5$.
...
As with the initial train-test split, the resampling strategy should respect the structure of the prediction problem. In classification, the folds are often stratified to preserve approximately the same outcome proportions. Grouped cross-validation keeps related observations in the same fold, while resampling for time-ordered data preserves temporal order.

Cross-validation is part of model development and should therefore be carried out entirely within the training set. The test set remains outside model comparison, hyperparameter tuning, feature selection, and other development decisions until final assessment.
...
Figure [-@fig-data-prep-cross-validation-2] shows how cross-validation is carried out within the training set while the test set remains separate from model development.
...
Data-dependent preparation must also be incorporated within cross-validation: in each iteration, preparation rules are learned from the training folds and then applied to the corresponding validation fold. Otherwise, validation observations may influence model development indirectly. This issue is developed further in Section [-@sec-data-prep-data-leakage].

After the preparation workflow, model, and hyperparameters have been selected, the chosen preparation rules and final model are fitted using the full training set. The fitted preparation rules are then applied to the test data before final assessment.

This workflow is used in Chapter [-@sec-knn-choose-k], where cross-validation is applied to select the number of neighbors in a *k*-Nearest Neighbors model. It is also used in the `adult` case study at the end of this chapter.

## Preventing Data Leakage {#sec-data-prep-data-leakage}
...
## Handling Unusual Values {#sec-data-prep-outliers}
...
## Handling Missing Values {#sec-data-prep-missing-values}
...
### Missing Values and Missingness Mechanisms {.unnumbered .unlisted}
