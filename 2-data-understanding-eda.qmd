...
A dataset does not explain itself. Values that appear ordinary may encode missing information, unusual observations may reflect errors or meaningful rare cases, and seemingly different features may contain overlapping information. Before drawing conclusions or building predictive models, we therefore need to understand what the observations represent, how the features are defined and measured, whether the recorded values are plausible, and what patterns and relationships are present. Developing this understanding is the purpose of the *Data Understanding and Exploration* stage of the Data Science Workflow.

This stage follows Data Acquisition in the workflow introduced in Figure [-@fig-ds_DSW] and provides the empirical foundation for Data Preparation for Modeling (Chapter [-@sec-chapter-data-preparation]). Its purpose is not simply to produce summaries and plots, but to develop a reliable understanding of the data before analytical decisions are made. This requires combining knowledge of how the data were defined and collected with numerical summaries, visualizations, and contextual reasoning to examine data quality, distributions, relationships, and potential limitations.

Exploratory data analysis (EDA) is a central component of this stage. It is primarily investigative rather than confirmatory: it helps us discover patterns, identify potential problems, compare groups, examine relationships, and formulate questions for further analysis. These findings may motivate later data preparation, statistical inference, or predictive modeling, but they should not be treated as final statistical or causal conclusions. Formal tools for quantifying uncertainty and drawing conclusions beyond the observed data are introduced later in Chapter [-@sec-chapter-statistics].

Data understanding and exploration are iterative rather than strictly linear. An unexpected value may require us to revisit the data source, an apparent pattern may prompt closer examination of particular observations or subgroups, and a feature whose meaning is unclear may require further documentation. An important distinction must therefore be maintained between identifying and understanding a data issue and deciding how to address it. This chapter focuses on the former; decisions about imputation, transformation, encoding, feature representation, and other procedures used to create a model-ready dataset belong to the next stage of the workflow, Data Preparation for Modeling, addressed in Chapter [-@sec-chapter-data-preparation].

### What This Chapter Covers {.unnumbered .unlisted}

This chapter develops a systematic approach to data understanding and exploration. We begin by examining how visualizations can reveal patterns, support clear data storytelling, and generate questions for further investigation. We then consider the types and analytical roles of features, the relationship between their conceptual meaning and technical representation in R, and the interpretation of numerical distributions. We also examine how unusual observations and implausible values can be identified and investigated using graphical, numerical, and contextual information. Readers who need a review of R fundamentals, including data frames, indexing, data manipulation, and basic visualization, can consult Appendix [-@sec-chapter-r-foundations].

These principles are applied in a guided case study using the `churn` dataset from the **liver** package. The case study begins with problem definition, data acquisition, initial inspection, and assessment of data quality. We then explore categorical and numerical features and investigate relationships and potential redundancy among features. Throughout the analysis, numerical summaries, visualizations, and contextual reasoning are used together to identify meaningful patterns and limitations while distinguishing exploratory associations from conclusions that would require formal statistical or causal evidence.

By the end of the chapter, readers should be able to formulate appropriate questions for data understanding and exploration, identify feature types and analytical roles, assess data quality, and investigate unusual or implausible observations. They should also be able to select suitable numerical and graphical summaries, compare patterns across groups, examine relationships and potential redundancy among features, and communicate exploratory findings clearly without overstating their implications. Finally, readers should be able to use insights obtained during exploration to motivate later decisions during Data Preparation for Modeling.

## Seeing Patterns and Data Storytelling {#sec-eda-seeing-patterns}

Data exploration helps reveal patterns, differences, and potential limitations that are difficult to recognize from raw observations alone. Visualizations are particularly valuable because they can make relationships, unusual observations, group differences, and changes over time visible, while also drawing attention to questions that deserve closer investigation.

A well-known example is Hans Rosling’s TED Talk *New insights on poverty* [@rosling2007poverty], in which demographic and economic data are used to reveal and communicate patterns of global development over time. Figure [-@fig-EDA-fig-1] presents a related visualization based on the `gapminder` dataset from the **liver** package. It compares GDP per capita and life expectancy across countries in 1950 and 2019. Countries are distinguished by world region, and point size is proportional to population.
...
Several patterns are immediately visible. Between 1950 and 2019, both GDP per capita and life expectancy generally increased, although substantial differences remained across countries and world regions. The figure also suggests that countries with higher GDP per capita tend to have higher life expectancy. At the same time, countries with similar levels of GDP may differ considerably in life expectancy, indicating that the relationship is not uniform.

The visualization therefore provides both information and questions. How strong is the relationship between GDP per capita and life expectancy? Which countries depart from the overall pattern, and do regional differences help explain some of this variation? Such questions can guide further exploration. The observed association is descriptive, however, and should not be interpreted as evidence that changes in GDP per capita directly cause changes in life expectancy.

Visual exploration also plays an important role in communicating what we learn from data. In this context, *data storytelling* means selecting and organizing relevant graphical and numerical evidence around a clear question so that the main patterns, their context, and their limitations can be understood. It does not mean imposing a predetermined narrative on the data or presenting only the evidence that supports a preferred conclusion. Effective data storytelling distinguishes between what is directly observed, how the observed patterns may be interpreted, and which questions remain unresolved.

Seeing and communicating a pattern is only the beginning. Data understanding and exploration also require us to investigate what the pattern represents, whether it is consistent across the dataset, which observations depart from it, and what additional information may be needed to interpret it. The next step is therefore to translate visible patterns into systematic exploratory questions.

### From Patterns to Questions {.unnumbered .unlisted}

To move from observation to systematic exploration, we need to translate visible patterns into questions that can be investigated using the data. These questions can be organized around three complementary levels: understanding the dataset as a whole, examining individual features, and investigating relationships among features. Exploratory data analysis (EDA) provides numerical and graphical tools for addressing many of these questions, while contextual information helps determine how the resulting patterns should be interpreted.

At the dataset level, we ask what each observation represents, how and when the data were collected, and which population or process the data describe. We identify the unit of analysis and determine whether the dataset contains a target feature, predictors, identifiers, derived measurements, or other features with special analytical roles. These questions help establish whether the available data are appropriate for addressing the problem of interest and which limitations should be considered during exploration.

At the feature level, we examine what each feature means, how it is represented in R, and which values it can reasonably take. We investigate its distribution and range, assess the presence of missing or unspecified information, and look for unusual observations. Relevant questions include whether a numerical feature is symmetric or skewed, whether a categorical feature contains rare or placeholder levels, and whether extreme values represent errors, valid rare cases, or meaningful subgroups.

At the relationship level, we examine how features vary together. We may ask whether a numerical feature differs across groups, whether two numerical features are associated, or whether two categorical features occur together systematically. We also investigate whether features contain overlapping information, are mathematically derived from one another, or reveal patterns only when considered jointly. When a target feature is available, we may also examine how other features vary across target groups.

The interpretation of exploratory findings depends on the context in which the data were defined and collected. A large numerical value may be unusual but valid in one application and implausible in another. Similarly, a value such as `"unknown"` may be stored as an ordinary categorical level in R while representing missing or unspecified information. The choice of exploratory tools should therefore follow from both the question and the feature types involved. Numerical features may be examined using summary statistics and distributional plots, categorical features using frequencies and bar plots, and relationships among features using grouped summaries, scatter plots, contingency tables, or measures of association. These tools should be interpreted together with feature definitions, measurement units, the data-collection process, and relevant domain knowledge. Systematic exploration therefore depends not only on selecting appropriate tools, but also on understanding what the data represent and what questions they can reasonably address.
...
It is equally important to consider a feature’s analytical role. A feature may serve as a predictor, a target, an identifier, or a quantity derived from other features. These roles affect how the feature should be explored and interpreted. For example, a customer identifier may be stored as a number, but it represents a label rather than a measurable quantity and should not be treated as an ordinary numerical feature.

Understanding both feature type and analytical role also provides a foundation for later Data Preparation for Modeling. Numerical features may eventually require transformation or scaling, while categorical features may need encoding or other forms of preparation. Such decisions should be made only after the meaning, representation, and observed behavior of the relevant features have been examined carefully.
...
## Case Study: Exploring the `churn` Dataset {#sec-eda-EDA-churn}
...
Although the focus of this case study is the *Data Understanding and Exploration* stage, we briefly revisit Data Acquisition to establish the source, structure, and context of the data before beginning detailed exploration. The `churn` dataset, available in the **liver** package, contains demographic, account-related, behavioral, and financial information about credit card customers. The target feature is `churn`, which indicates whether a customer closed their credit card account (`"yes"`) or remained active (`"no"`). At this stage, the target is used to guide exploration rather than to build a predictive model.
...
The dataset is stored as a `data.frame` with `r nrow(churn)` observations and `r ncol(churn)` features. Each row represents one credit card customer, establishing the customer as the unit of analysis. Each column records a demographic characteristic, account attribute, or measure of customer behavior or activity.
...
The feature `customer_ID` illustrates why technical representation should not be confused with analytical meaning. Although it is stored numerically in R, it serves as an account identifier rather than as a quantitative measurement. Numerical summaries such as its mean or standard deviation therefore have no substantive interpretation. Its role is determined by what the feature represents, not by the fact that R stores it as a number.

Some numerical features are derived from others. For example, `available_credit` is calculated from `credit_limit` and `revolving_balance`, while `utilization_ratio` expresses revolving balance relative to credit limit. These derived features summarize aspects of credit behavior, but their mathematical relationships with their component features mean that they contain overlapping information. We return to these relationships later when examining associations and potential redundancy among features.

Initial inspection therefore involves more than identifying feature names and R classes. It establishes the unit of analysis, clarifies the target and other analytical roles, distinguishes conceptual feature types from technical representations, and identifies features whose meanings are linked by construction. With the basic structure and interpretation of the dataset established, the next step is to assess whether the recorded values raise any data-quality concerns.

> *Practice:* Run `dim(churn)`, `names(churn)`, and `head(churn)`. How does each function complement the information provided by `str(churn)`? Identify the unit of analysis, the target feature, the customer identifier, and at least one feature derived from other measurements.

### Assessing Data Quality {.unnumbered .unlisted}

Once the structure and analytical roles of the features are understood, the next step is to assess the quality of the recorded data. At this stage, we look for missing or unspecified information, implausible values, and other indications that the data may not fully represent the quantities or categories they are intended to measure. The objective is to identify, investigate, and document such issues rather than decide immediately how they should be handled.

We first use the `find.na()` function from the **liver** package to check for values represented explicitly as `NA` in R:

```{r}
find.na(churn)
```

The output indicates that the dataset contains no values coded as `NA`. This does not necessarily mean, however, that all information is complete. Missing or unspecified values may be encoded using placeholder categories or special numerical values that R treats as ordinary observations. We therefore inspect selected categorical features more closely:
...
During exploration, we retain `"unknown"` as an explicit level so that these observations remain visible in summaries and visualizations. This allows us to determine how frequently information is unspecified and to examine whether the corresponding observations show patterns that differ from those in the substantive categories. At this stage, retaining `"unknown"` is an exploratory choice rather than a final decision about how these values should be represented in later analyses.

We also inspect the numerical features for unusual ranges and potentially implausible values. The `summary()` function provides a useful initial overview:

```{r}
summary(churn)
```

The numerical summaries, together with the visualizations presented later in the case study, do not reveal clearly implausible numerical values in the `churn` dataset. Some features are skewed or contain relatively extreme observations, but these values appear plausible in the context of customer credit and transaction behavior. An observation should therefore not be classified as erroneous simply because it lies far from the center of a distribution.

This assessment highlights an important distinction between technical completeness and substantive data quality. A dataset may contain no values coded as `NA` while still containing missing or unspecified information, and an extreme numerical value may be unusual without being implausible. Data quality must therefore be assessed using the technical representation of the data together with feature definitions, plausible ranges, relationships among features, and contextual knowledge.

The issues identified during this stage are documented for later consideration. Decisions about recoding placeholder values, imputing missing information, transforming features, or applying other preparation procedures belong to Data Preparation for Modeling in Chapter [-@sec-chapter-data-preparation]. With the main data-quality issues identified, we can now proceed to a more detailed exploration of categorical and numerical features and the relationships among them.

> *Practice:* Examine the output of `summary(churn)`. Which categorical features contain the level `"unknown"`? Which numerical features have large ranges or skewed distributions? Do any values appear clearly implausible based on the available information, and what additional evidence would be needed before classifying them as errors?
...
```{r}
addmargins(table(churn$churn, churn$gender, dnn = c("Churn", "Gender")))
```
...
This chapter introduced the *Data Understanding and Exploration* stage of the Data Science Workflow and presented exploratory data analysis (EDA) as a central component of that stage. Beginning with the `gapminder` example, we saw how visualizations can reveal patterns, differences, and relationships that are difficult to recognize from raw observations alone. We also considered the role of data storytelling in communicating these patterns clearly while distinguishing what is directly visible in the data from interpretations that require further evidence. Exploratory patterns therefore serve not only as findings to communicate, but also as starting points for questions that guide deeper investigation.

A systematic exploration begins by understanding the dataset as a whole, examining individual features, and investigating relationships among features. We distinguished numerical and categorical features, considered their analytical roles and technical representations in R, and examined how distribution shape affects the interpretation of numerical summaries. Using the `diamonds` dataset, we also distinguished statistically unusual observations from implausible values and showed why unusual observations should be investigated using contextual information, related features, and domain knowledge rather than removed automatically.

The `churn` case study demonstrated how these principles can be applied to a practical problem. Some demographic and product-related features showed limited differences across churn groups, whereas behavioral features such as customer-service contacts, transaction activity, and changes in spending revealed clearer exploratory patterns. The analysis also illustrated the importance of considering group sizes, placeholder categories such as `"unknown"`, class imbalance, distributional overlap, and relationships among features when interpreting exploratory results.

Finally, examining features jointly revealed associations and potential redundancy that were not apparent from isolated summaries. These findings provide the basis for decisions in the next stage of the workflow, Data Preparation for Modeling, introduced in Chapter [-@sec-chapter-data-preparation], where issues identified during exploration are translated into appropriate preparation decisions. They also provide an exploratory foundation for the formal statistical inference methods introduced later in Chapter [-@sec-chapter-statistics].
...
15. Why is it important to distinguish between what is directly visible in an exploratory visualization and conclusions that would require statistical or causal evidence?
