...
> *Practice*: Load the `churn` dataset from the **liver** package (see [Section -@sec-ch1-import-data]). Then use `str(churn)` to inspect its structure. Which variables are numeric, character, or factors? Try using `class()` and `typeof()` on a few columns to explore how R understands them.
