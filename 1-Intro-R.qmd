...
R provides several types of operators for performing calculations, making comparisons, and combining logical conditions. These operators form the foundation of most expressions in R and are used throughout data analysis workflows.

Arithmetic operators include `+`, `-`, `*`, `/`, and `^`, which represent addition, subtraction, multiplication, division, and exponentiation, respectively. Their behavior follows standard mathematical rules:
...
Relational operators compare values and return logical results (`TRUE` or `FALSE`). The most common are `==` (equal to), `!=` (not equal to), `<`, `>`, `<=`, and `>=`:
...
Logical operators combine or invert logical values—`&` (and), `|` (or), and `!` (not)—and are particularly useful when filtering data or defining conditions. Figure [-@fig-logic-operators] illustrates their behavior:
...
A concise overview of these operators, grouped by their function, is presented in Table [-@tbl-common-operators]. These operators are fundamental to working with data in R, particularly when selecting subsets, defining conditions, or constructing logical expressions.
Beyond these basic arithmetic and logical operators, R also provides special operators, introduced next.
...
Overview of arithmetic, relational, and logical operators in R, along with assignment, membership, and sequence operators frequently used in data analysis
:::
:::
...
Readers who are new to R do not need to master these operators immediately. The aim here is simply to make you familiar with them, since they frequently appear in online examples and in code generated by AI tools such as ChatGPT. In my experience, students often encounter these operators when seeking help with R, which is why a short overview is included in this book.

Pipes express a sequence of operations from left to right. Instead of nesting functions, you write one step per line. This makes data manipulation code more structured and easier to read. The two pipe operators serve the same purpose but differ slightly in syntax and origin.
...
This can be read as: take `mtcars`, filter to six cylinders, then compute a summary. In practice, it is good style to keep each pipeline short and meaningful—for example, `filter → mutate → summarise`. When a step becomes complex, it is clearer to store it in a variable rather than extending the chain.
...
Although this book focuses on data science applications rather than programming conventions, understanding these operators will help you write modern, reproducible R code. They make analyses concise, expressive, and transparent. In later chapters, they will appear selectively—used when they enhance clarity without obscuring the logic of the analysis.
