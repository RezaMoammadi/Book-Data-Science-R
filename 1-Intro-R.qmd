...
Pipes express a sequence of operations from left to right. Instead of nesting functions, you write one step per line, which improves the readability of data manipulation code. The two pipe operators serve the same purpose but differ slightly in syntax and origin.

### The `%>%` Operator (from *magrittr*/*dplyr*) {.unnumbered}

The `%>%` operator passes the left-hand side (LHS) as the first argument to the function on the right-hand side (RHS). It is part of *magrittr* and widely used in *dplyr* workflows.
...
This can be read as: take `mtcars`, filter to six cylinders, then compute a summary.

Two useful patterns demonstrate its flexibility:

```r
# Pipe into a non–first argument using the placeholder `.`
mtcars %>% mutate(hp_scaled = hp / 100) %>% quantile(., probs = 0.9)

# Apply a short inline expression
mtcars %>% { .[.$mpg > 25, c("mpg", "hp")] }
```

Good practice with `%>%`:

* Keep each step short and purposeful (for example, `filter → mutate → summarise`).
* If a step becomes complex, assign it to a variable rather than extending the pipeline.
* Prefer named arguments for readability.

The next operator, `|>`, was introduced in base R and provides a similar but simpler mechanism for chaining functions without loading additional packages.

### The `|>` Operator (base R, R ≥ 4.1) {.unnumbered}

The base pipe `|>` behaves much like `%>%`, passing the LHS as the first argument to the RHS function. Because it is built into R, no extra packages are needed.
...
> **Readability tip:** Pipes make code clearer, but they are not mandatory. Use them when they simplify logic, and break pipelines that become long or difficult to follow.

While pipes simplify the flow of data between functions, the `::` operator serves a different role: it identifies exactly which package a function belongs to, improving transparency.

### Calling Functions from a Specific Package with `::` {.unnumbered}

When multiple packages define functions with the same name, `::` ensures that you call the intended one without loading the entire package.

```r
# Call a function explicitly from a package's namespace
liver::partition()

# Build a plot using ggplot2 functions without loading the package
ggplot2::ggplot(mtcars, ggplot2::aes(mpg, hp)) + ggplot2::geom_point()
```

This approach clarifies dependencies and supports reproducibility, especially in collaborative projects.
(Advanced users may encounter `:::`, which accesses non-exported functions, but this is generally discouraged.)

Quick comparison of the three operators:

* `%>%` (*magrittr*): Common in tidyverse workflows; supports the `.` placeholder and inline expressions.
* `|>` (base R): Built-in and dependency-free; use anonymous functions for non–first arguments.
* `::`: Calls a function explicitly from a specific package, improving clarity and reproducibility.

Although this book focuses on data science applications rather than programming conventions, familiarity with these operators is valuable. They make code concise, expressive, and easier to share. Later chapters use them selectively to balance clarity with accessibility.
...
Have you ever looked at a dataset and asked, *What patterns are hiding here?* Visualization helps you answer that question by turning raw numbers into compelling, informative graphics. In data science, seeing is often understanding. Whether you are identifying patterns, checking assumptions, or presenting results, effective visuals transform numbers into insight. As you will see in Chapter [-@sec-ch4-EDA], exploratory data analysis (EDA) relies heavily on visual summaries to uncover trends, outliers, and relationships.
...
Aesthetics in **ggplot2** define how data variables are visually mapped to features such as color, size, shape, and transparency. These mappings are specified inside the `aes()` function and allow your plot to visually reflect differences in the data. For example, to map the color of each point to the number of cylinders in a car:
...
When aesthetics are placed *inside* `aes()`, they respond to the data. If you want to apply a constant appearance—such as making all points blue or using a custom color—set the attributes *outside* of `aes()`:
...
You can specify colors either by name (e.g., `color = "blue"`) or by hexadecimal code (e.g., `color = "#1B3B6F"`). Hex codes offer precise control and ensure consistent visual design across figures. Try experimenting with both naming styles to see how subtle color changes affect the tone and readability of your plots. The example above uses a medium-dark blue tone (`"#1B3B6F"`) that appears throughout this book’s figures, helping maintain a clean and cohesive visual style.

Setting aesthetics outside `aes()` creates a fixed appearance: all points share the same color, shape, and size. Since this appearance is not linked to the data, **ggplot2** does not generate a legend.

> *Try it yourself*: In the `churn` dataset, can you map color to the `churn` variable and size to `custserv.calls` in a scatter plot of `day.mins` versus `eve.mins`? Try changing the color palette or using a hex code instead of a color name. How do these visual choices affect interpretation?

With just a few core components, such as `geom_point()`, `geom_smooth()`, and `aes()`, you can construct expressive and insightful graphics. In Chapter [-@sec-ch4-EDA], we will expand on this foundation to explore distributions, relationships, and trends as part of the exploratory data analysis process. For more details, see the [**ggplot2** documentation](https://ggplot2.tidyverse.org). If you are curious about interactive visualizations, explore packages such as [**plotly**](https://plotly.com/r) or [**Shiny**](https://CRAN.R-project.org/package=shiny).
