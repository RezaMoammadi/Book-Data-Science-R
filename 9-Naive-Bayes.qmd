...
```{r fig-bayes-theorem, echo = FALSE, out.width = "80%", fig.cap = "Conceptual illustration of Bayes’ theorem. Prior beliefs and observed data are combined through the likelihood to produce the posterior distribution."}
...
> *Practice:* Using the same contingency table, compute the probability that a loan application is `approved` given that the applicant has `education = not-graduate`. How does this probability compare with the value obtained for applicants with `education = graduate`? *Hint:* Identify the relevant counts in the table and apply Bayes’ theorem. You may verify your result in R.
...
$$
P(Y = y \mid X_1, \dots, X_p)
=
\frac{P(X_1, \dots, X_p \mid Y = y) P(Y = y)}
{P(X_1, \dots, X_p)}.
$$

This expression combines two key components. The term $P(Y = y)$ represents the prior probability of class $y$, which reflects how common that outcome is in the population. The term $P(X_1, \dots, X_p \mid Y = y)$ represents the likelihood, which measures how probable the observed predictor values are if the observation truly belongs to class $y$. Together, these quantities determine the posterior probability $P(Y = y \mid X_1, \dots, X_p)$.
...
To train the model, we specify a formula in which `loan_status` is the outcome variable and the remaining variables serve as predictors:

```{r}
formula = loan_status ~ no_of_dependents + education + self_employed + income_annum + loan_amount + loan_term + cibil_score + residential_assets_value + commercial_assets_value + luxury_assets_value + bank_asset_value
```
...
#### Hands-On Practice: Naive Bayes with the `churn_mlc` Dataset {.unnumbered .unlisted}
...
##### Training and Evaluation {.unnumbered .unlisted}
...
#### Hands-On Practice: Naive Bayes with the `churn` Dataset {.unnumbered .unlisted}
...
37. Partition the data into 80% training and 20% test sets using `partition()` from **liver**. 
...
#### Critical Thinking {.unnumbered .unlisted}
