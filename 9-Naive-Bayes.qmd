...
Noe that the `nr.loans` variable is a count with values such as 0, 1, and 3. While the default setting uses a Gaussian distribution, it may be worth experimenting with the `usepoisson = TRUE` option to see whether a Poisson distribution offers a better fit. As an exercise, you are encouraged to compare model performance with and without this option.
...
### Hands-on Implementation with the Churn Dataset {.unnumbered}
...
### Real-World Application and Critical Thinking {.unnumbered}
