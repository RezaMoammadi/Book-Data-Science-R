...
The objective of clustering is to achieve *high intra-cluster similarity* and *low inter-cluster similarity*. This principle is illustrated in @fig-ch14-cluster-1, where compact, well-separated groups correspond to an effective clustering solution.

```{r fig-ch14-cluster-1, echo = FALSE, out.width = "70%", fig.cap = "Clustering algorithms aim to minimize intra-cluster variation while maximizing inter-cluster separation."}
...
### How Do Clustering Algorithms Measure Similarity? {.unnumbered .unlisted}
...
```{r}
#| label: fig-ch14-euclidean-distance
#| out.width: "70%"
...
ggplot(points_df, aes(x = x, y = y)) +
  geom_text(aes(label = label), vjust = -1.3, hjust = .7, size = 5, color = "#4C78A8") +
  geom_segment(data = segment_df,
               aes(x = x, y = y, xend = xend, yend = yend),
               color = "#C76E00", size = 1) +
  annotate("text", x = 5.4, y = 4.6,
...
As illustrated in @fig-ch14-euclidean-distance, Euclidean distance captures the geometric separation between two points. For Point A=(2, 3) and Point B=(6, 7), this distance is $$
...
knitr::include_graphics("images/ch14_example_fig_2.png")
```

After the initial assignment, the algorithm updates the cluster centers by computing the centroid of each group. These updated centroids are shown in the left panel of @fig-ch14-example-3. As the centers move, the Voronoi boundaries shift, causing some observations to be reassigned, as shown in the right panel.

```{r}
#| label: fig-ch14-example-3
#| out.width: "100%"
#| fig-cap: "Second iteration of K-means clustering. Updated cluster centroids and revised assignments."
#| echo: false

knitr::include_graphics("images/ch14_example_fig_3.png")
```

This process of reassignment and centroid update continues iteratively. With each iteration, the cluster structure becomes more stable, as illustrated in @fig-ch14-example-4. Eventually, no observations change clusters, and the algorithm converges, producing the final clustering shown in @fig-ch14-example-5.

```{r}
#| label: fig-ch14-example-4
#| out.width: "100%"
#| fig-cap: "Later iteration of K-means clustering, showing further refinement of cluster assignments."
#| echo: false

knitr::include_graphics("images/ch14_example_fig_4.png")
```

```{r}
#| label: fig-ch14-example-5
#| out.width: "55%"
#| fig-cap: "Final clustering after convergence, with stable cluster assignments."
#| echo: false

knitr::include_graphics("images/ch14_example_fig_5.png")
```
...
## Selecting the Optimal Number of Clusters {#sec-ch14-kmeans-choose}
...
This idea is illustrated in @fig-ch14-elbow, which plots the total within-cluster sum of squares (WCSS) against the number of clusters. A pronounced bend in the curve suggests a value of $k$ that balances model simplicity with explanatory power.
...
## Case Study: Segmenting Cereal Brands by Nutrition {#sec-ch14-case-study}
...
### Selecting the Number of Clusters {.unnumbered .unlisted}
...
#### Interpreting the Results {.unnumbered .unlisted}
...
#### Hands-On Practice: K-mean with the `red_wines` Dataset {.unnumbered .unlisted}
...
##### Data Preparation and Exploratory Analysis {.unnumbered .unlisted}
...
##### Applying k-means Clustering {.unnumbered .unlisted}
