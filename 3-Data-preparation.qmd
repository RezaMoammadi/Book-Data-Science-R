...
## Identifying Feature Types {#sec-ch3-feature-types}
...
#| fig-cap: "Overview of common feature types used in data analysis, including numerical (continuous and discrete) and categorical (ordinal, binary, and nominal) variables."
...
The resulting plot confirms that missing values occur only in three variables: `workclass` with `r sum(is.na(adult$workclass))` entries, `occupation` with `r sum(is.na(adult$occupation))` entries, and `native_country` with `r sum(is.na(adult$native_country))` entries.
...
The output shows that the minimum value is `r min(adult$capital_loss)` and the maximum is `r max(adult$capital_loss)`. More than 75% of observations are equal to zero. The median, `r median(adult$capital_loss)`, is substantially lower than the mean, `r round(mean(adult$capital_loss), 2)`, indicating a right-skewed distribution driven by a small number of large values.
